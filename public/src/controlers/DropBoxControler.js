class DropBoxControler {
    ...
    constructor() {
        ...
        this.buttonSendFileEl = document.getElementById('btn-send-file');
        this.inputFilesEl = document.getElementById("files");
        this.snackBarEl = document.getElementById("react-snackbar-root");
        this.progressBar = this.snackBarEl.querySelector('.mc-progress-bar-fg');
        ...
        this.onselectedchange = new Event('onselectedchange');
        this.startUploadTime;
        this.btnNewFolder = document.querySelector('#btn-new-folder')
        this.btnDelete = document.querySelector('#btn-delete');
        this.btnRename = document.querySelector('#btn-rename');
        this.lastFolder;
        this.conectToFirebase();
        this.initEvents();
        this.openFolder();
    }
    ...
    conectToFirebase() {
        ...
        var firebaseConfig = {
            apiKey: "AIzaSyD1o2APPiWvECU5HkXJc_4DRWL0RL2Z3YY",
            authDomain: "drop-box-clone-74d39.firebaseapp.com",
            databaseURL: "https://drop-box-clone-74d39-default-rtdb.firebaseio.com",
            projectId: "drop-box-clone-74d39",
            storageBucket: "drop-box-clone-74d39.appspot.com",
            messagingSenderId: "131774725827",
            appId: "1:131774725827:web:4eac8372140bca4592ac76",
            measurementId: "G-E04P4JWTQM"
        };
        // Initialize Firebase
    ...
    getItensSelected() {
        //retorna todos os filhos que possuem essa class 'nodeList'
    ...
    removeTask() {
        ...
        this.getItensSelected().forEach(li => {
            ...
            promises.push(new Promise((resolve, reject) => {
                ...
                if (file.type === 'folder') {
                    ...
                    this.removeFolder(this.currentFolder.join('/'), file).then(response => {
                        resolve({
                            fields: {
                                key
                            }
                    ...
                    this.removeFile(this.currentFolder.join('/'), file).then(response => {
                        resolve({
                            fields: {
                                key
                            }
    ...
    removeFolder(ref, file) {

        return new Promise((resolve, reject) => {
            let folderRef = this.getFirebaseRef(ref + '/' + file.name);

            folderRef.on('value', snapshots => {
                ...
                snapshots.forEach(snap => {
                    ...
                    if (data.type === 'folder') {
                        ...
                        this.removeFolder(ref + '/' + file.name, data.name).then(() => {
                        ...
                        this.removeFile(ref + '/' + file.name, data).then(() => {
    ...
    removeFile(ref, file) {
        ...
        let fileRef = firebase.storage().ref(ref).child(file.name);
    ...
    initEvents() {
        ...
        this.btnNewFolder.addEventListener('click', event => {
            let newFolder = prompt('Nome da nova pasta');
        ...
        this.btnRename.addEventListener('click', event => {
            //o dataset só é acessivel em cada item, por isso estou pegando [0]
            let li = this.getItensSelected()[0];
            let file = JSON.parse(li.dataset.file);
            let name = prompt('Informe o novo nome', file.name);
        ...
        this.listFilesEl.addEventListener('onselectedchange', event => {

            switch (this.getItensSelected().length) {
                case 0:
        ...
        this.buttonSendFileEl.addEventListener('click', event => {
            this.inputFilesEl.click();
        });

        this.inputFilesEl.addEventListener('change', event => {

            this.buttonSendFileEl.disabled = true;

            this.uploadTask(event.target.files).then(responses => {

                responses.forEach(response => {

                    //console.log(response.ref._service.app.options.databaseURL);
                    console.log(response);

                    let path = this.getUrlFirebase(response.fullPath, response.name);
    ...
    uploadComplete() {
        ...
        this.buttonSendFileEl.disabled = false;
    }
    ...
    ajax(url, mhetod = 'GET', formData = new FormData(), onprogress = function () {}, onuploadstart = function () {}) {

        return new Promise((resolve, reject) => {

            let ajax = new XMLHttpRequest();

            ajax.open(mhetod, url);

            //função a ser executada quando a solicitação http for concluída com êxito
            ajax.onload = event => {
                try {
    ...
    updateProgressBar(event, file) {
        ...
        let porcent = parseInt((load / total) * 100);
        ...
        this.timeLeftEl.innerHTML = this.convertOfMiliseconds(Math.round(timeLeft));

    }

    convertOfMiliseconds(miliseconds) {
        ...
        if (seconds > 60) {
            seconds = seconds % 60;
            return `${minute} minuto/s e ${seconds} segundos`
        } else {
    ...
    getIconsUpload(file, key) {

        let li = document.createElement('li');
        li.innerHTML = `
                ${this.getNameFile(file)}
    ...
    addEventLi(li) {
        ...
        li.addEventListener('click', event => {
            ...
            let firstSelected = this.listFilesEl.querySelector('.selected');
            ...
            if (event.shiftKey) {
                ...
                lis.forEach((el, index) => {
                    ...
                    if (el === firstSelected) listSelecteds.push(index);
                    if (li === el) listSelecteds.push(index);

                });

                listSelecteds = listSelecteds.sort();

                lis.forEach((el, index) => {

                    if (index >= listSelecteds[0] && index <= listSelecteds[1]) {
                        el.classList.add('selected');
                    }

                });

                this.listFilesEl.dispatchEvent(this.onselectedchange);
    ...
    readyFilesFirebase() {
        ...
        this.getFirebaseRef().on('value', snapshot => {
            ...
            snapshot.forEach(snapItem => {
                ...
                if (data.type) {
                    this.listFilesEl.appendChild(this.getIconsUpload(data, key));
                }
            });
